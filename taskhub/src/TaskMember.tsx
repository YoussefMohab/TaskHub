...
import { FaHand, FaPhotoFilm } from "react-icons/fa6";
...
function TaskMember() {
  ...
  function actuleSubmit(taskId: any) {
    const formData = new FormData();
    formData.append("type", type);
    formData.append("taskId", taskId || ""); // Ensure projectId is not null
    ...
    axios
      .post(`${import.meta.env.VITE_TASK_API}/actulesubmit`, formData, {
        headers: {
          Authorization: `UserLoginToken ${token}`,
        },
      })
                ...
                <div style={{marginTop: '30px', marginBottom:'50px'}}>
                  ...
                  {showtask[task._id] && (
                    ...
                    <div className="taskdetails" style={{zIndex: '0'}}>
                      ...
                      {task.image && (
                  <img
                    src={`data:image/jpeg;base64,${task.image.imageURL}`}
                    alt={task.image.imageName}
                    style={{
                      maxWidth: "50%",
                  ...
                  />
                          ...
                          {task.file &&  (
                  <a
                    href={`data:application/octet-stream;base64,${task.file.fileURL}`}
                    download={task.file.fileName}
                    style={{
                      display: "block",
                      color: "#0366d6",
                      textDecoration: "none",
                ...
                {showPopup && (
                  <div className="popuprequests">
                    <center>
                      ...
                      <span style={{color: 'grey', fontSize: '15px',marginLeft:'-100px'}}>upload your task:</span>
                      <select 
                style={{ position: 'absolute', right: '40px', padding: '5px', backgroundColor: 'lightgrey'}}
                value={type}
                onChange={async (e) => {
            ...
            <div>
              ...
              <div style={{ margin: "20px", color: 'black' }}>
                ...
                {nameoffile}
                      ...
                      <button onClick={async () => {setShowPopup(false); await setType("");}}>Close</button>
                    </center>
                  </div>
                )}
