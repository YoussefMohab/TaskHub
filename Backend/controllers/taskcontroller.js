...
exports.actulesubmit = async (req, res) => {
  try {
    const { type, taskId } = req.body;
    const task = await Task.findById(taskId);
    const user = await User.findOne({ "email.address": req.user.email });
    const taskID = task.assignedMember.member.toString();
    const userID = user._id.toString();
    ...
    if (taskID !== userID) {
      return res
        .status(400)
        .json({ success: false, message: "You are not assigned to this task." });
    }
    ...
    // Update the task with the file or image data
    await Task.findByIdAndUpdate(taskId, { $set: data }, { new: true });

    res.json({
      success: true,
      message: `${type} uploaded and saved to the task.`,
    });

    const project = await Project.findById(task.projectId);
    createNotification(
      project.leaderID,
      `Leader: created a new post in "${project.title}"`
    );
  } catch (error) {
    console.error("Error:", error.message);
    res.status(500).json({ success: false, message: "Internal Server Error" });
